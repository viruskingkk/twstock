...
# -*- coding: utf-8 -*-
"""
台股歷史資料批次下載器（離線股本週轉率版）
=================================================
用途：下載 TWSE / TPEx 個股日K，輸出給「六脈神劍選股系統.html」上傳使用。

這版修正：
1. --all 預設改抓「上市/上櫃普通股」公司基本資料名冊，避免把 ETF、ETN、債券、權證等非普通股商品一起塞進股票池。
2. history.csv 會直接輸出 capital 欄位（股本，億元），HTML 匯入後可離線計算週轉率，不必再靠瀏覽器即時抓股本。
3. 另外輸出 stock_meta.csv，方便檢查代號、名稱、產業、股本。
4. 若要抓全部可交易商品才使用 --all-products；若要額外納入 ETF/ETN，可用 --include-etf / --include-etn。
5. 上櫃股票優先走新版 TPEx endpoint，legacy st43_result 只當備援。
6. 若 TPEx 官方端點查無或被暫時限流，改走 Yahoo Finance chart API 備援。
7. 已知上市/上櫃市場時不再每檔都先打 TWSE 再打 TPEx，降低請求量。
...
def main():
    parser = argparse.ArgumentParser(description="台股歷史資料批次下載器（離線股本週轉率版）")
    ...
    parser.add_argument("--out", default="history.csv", help="歷史行情輸出CSV檔名（預設 history.csv）")
    parser.add_argument("--meta-out", default="stock_meta.csv", help="股票基本資料輸出CSV檔名（預設 stock_meta.csv；含 code/name/market/industry/capital）")
    ...
    # 輸出股票基本資料，方便檢查股本是否有抓到；history.csv 也會同步帶 capital 欄位。
    try:
        with open(args.meta_out, "w", newline="", encoding="utf-8-sig") as mf:
            mw = csv.writer(mf)
            mw.writerow(["code", "name", "market", "industry", "capital"])
            for item in targets:
                mw.writerow([
                    item.get("code", ""),
                    item.get("name", ""),
        ...
        print(f"已輸出股票基本資料 {args.meta_out}（含股本欄位，單位：億元）")
    except Exception as e:
        print(f"⚠️ stock_meta.csv 輸出失敗：{e}")
    ...
    with open(args.out, mode, newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        if write_header:
            writer.writerow(["code", "name", "market", "industry", "capital", "date", "open", "high", "low", "close", "volume"])
        for i, item in enumerate(targets):
            ...
            try:
                ...
                if not rows:
                    ...
                    continue
                for r in rows:
                    writer.writerow([code, name, source or market_hint, industry, capital, r["date"], r["open"], r["high"], r["low"], r["close"], r["volume"]])
                f.flush()
                ok_count += 1
                if provider == "yahoo":
                    yahoo_count += 1
    ...
    print(f"已寫入 {args.meta_out}")
    print("接下來把 history.csv 上傳到 HTML 的「②上傳離線下載的CSV資料」區塊即可；若CSV含 capital 欄位，週轉率會自動計算。")
